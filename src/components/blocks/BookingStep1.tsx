...
export function BookingStep1({
  ...
  const adultPrice = isPrivate
    ? (tour?.privatePriceAdult ?? 0)
    : (tour?.groupPriceAdult ?? 0);
  const childPrice = isPrivate
    ? (tour?.privatePriceChild ?? 0)
    : (tour?.groupPriceChild ?? 0);
  const totalPrice =
    form.adultCount * adultPrice + form.childrenCount * childPrice;

  const changeCount = (
    field: "adultCount" | "childrenCount",
    delta: number,
  ) => {
  ...
  const selectedSchedule = schedules.find(
    (s) => s.scheduleId === form.scheduleId,
  );
        ...
        <section className="bg-card rounded-2xl p-6 shadow-sm border border-border">
          <div className="mb-5">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Note: Only dates with available departure schedules can be
              selected. Other dates are disabled.
            </p>
          </div>
                ...
                <Calendar
                  ...
                  disabled={(d) => {
                    ...
                    const hasDeparture = schedules.some(
                      (s) =>
                        !s.isTemplate && s.departureAt?.startsWith(dateStr),
                    );
                    ...
                    const isWithinTemplate = schedules.some((s) => {
                      const isTemplateSchedule =
                        s.isTemplate === true ||
                        (!s.departureAt && s.startDate && s.endDate);
                        ...
                        .map((sch) => {
                          ...
                          return (
                            <button
                              key={sch.scheduleId}
                              onClick={() =>
                                onUpdateForm({ scheduleId: sch.scheduleId! })
                              }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                Join other travelers in your journey.
              </p>
            </button>
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                The entire tour is for you and your group. Flexible itinerary.
              </p>
            </button>
          </div>
        </section>
          ...
          <div className="space-y-5">
            {[
              ...
              },
              {
                ...
                sub: "Age 4–11",
                price: childPrice,
                field: "childrenCount" as const,
                min: 0,
              },
            ].map(({ label, sub, price, field, min }) => (
              <div
                key={field}
                className="flex items-center justify-between pb-5 border-b border-dashed border-border last:border-0 last:pb-0"
              >
                <div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    onUpdateForm({ customerName: e.target.value })
                  }
                />
                {errors.customerName && (
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  onUpdateForm({ pickupLocation: e.target.value })
                }
              />
            </div>
            <div>
            ...
            <div className="p-5">
              ...
              <div className="mt-5 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => onUpdateForm({ isDeposit: false })}
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-bold mt-1 ${!form.isDeposit ? "text-primary" : "text-foreground"}`}
                    >
                      {totalPrice.toLocaleString()} VND
                    </span>
                    {!form.isDeposit && (
                  ...
                  <button
                    type="button"
                    onClick={() => onUpdateForm({ isDeposit: true })}
                    className={`flex flex-col items-center p-3 rounded-xl border-2 transition-all text-center ${
                      form.isDeposit
                        ? "border-primary bg-primary/10 shadow-sm"
                        : "border-border hover:border-primary/40 hover:bg-secondary/30"
                ...
                {/* Info when deposit selected */}
                {form.isDeposit && (
                  <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 space-y-1 text-xs animate-in fade-in duration-200">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">
                        Pay now (30%)
                      </span>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-foreground">
                        {Math.round(totalPrice * 0.7).toLocaleString()} VND
                      </span>
                    </div>
                    <p className="text-[10px] text-muted-foreground pt-1 border-t border-primary/10">
                      Remaining balance due before the tour completion.
                    </p>
                  </div>
                )}
              </div>
