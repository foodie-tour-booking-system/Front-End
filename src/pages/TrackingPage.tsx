...
export function TrackingPage() {
  ...
  const [relocateState, setRelocateState] = useState<
    "idle" | "sending_otp" | "otp_sent" | "verifying" | "success"
  >("idle");
  ...
  const handleSearch = async (code?: string) => {
    ...
    try {
      ...
      try {
        const bookingDetails = await BookingService.getBookingById({
          bookingCode: searchCode,
        });
        if (bookingDetails.tourId) {
          const schs = await ScheduleService.getSchedules({
        ...
        console.error(
          "Could not fetch additional booking details for relocation.",
        );
      }
    } catch (err: any) {
      setError(
        "Sorry, we couldn't find any tour with this booking code. Please check your reference and try again.",
      );
      setTracking(null);
    } finally {
      setLoading(false);
  ...
  const handleRequestRelocate = async () => {
    ...
    setRelocateError("");
    try {
      const token = await BookingService.relocateBooking({
        bookingCode: tracking.bookingCode,
      });
          ...
          <div className="container mx-auto px-6 relative z-10 flex flex-col items-center">
            <div className="max-w-3xl w-full text-center mb-12">
              ...
              <h1 className="text-5xl lg:text-7xl font-black text-white mb-6 uppercase tracking-tighter italic">
                Track your{" "}
                <span className="text-primary not-italic">Adventure</span>
              </h1>
            ...
            <div className="w-full max-w-2xl bg-white/5 backdrop-blur-2xl p-2 rounded-[2.5rem] border border-white/10 shadow-2xl flex flex-col sm:flex-row gap-2">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <RefreshCw className="size-5 mr-3" /> Start Tracking
                  </>
                )}
              </Button>
            </div>
        ...
        {!tracking && !loading && !error && (
          <div className="container mx-auto px-6 py-24 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-muted-foreground text-lg max-w-md mx-auto leading-relaxed">
              Enter your booking reference to check live status, pickup location
                  ...
                  <div className="space-y-8">
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-muted-foreground font-bold text-sm">
                        ...
                        Departure:{" "}
                        {tracking.departureTime
                          ? new Date(tracking.departureTime).toLocaleString(
                              "vi-VN",
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      ...
                      <div className="flex flex-col gap-1.5">
                        ...
                        <p className="text-xl font-extrabold text-foreground italic">
                          {tracking.tourName}
                        </p>
                        <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed mt-1">
                          {tracking.tourDescription ||
                            "Embark on an unforgettable journey through the heart of the waterways."}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10 space-y-8">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <label className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1 block">
                          Pickup Zone
                        </label>
                        <p className="text-lg font-bold leading-tight">
                          {tracking.pickupLocation || "Ho Chi Minh Pier #01"}
                        </p>
                      </div>
                    </div>
                ...
                <div className="bg-card border border-border rounded-[2.5rem] p-10 shadow-xl shadow-slate-200/50 dark:shadow-none flex-1">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <p className="text-muted-foreground font-medium text-sm">
                        Need to move to another time? Request a relocation.
                      </p>
                    </div>
                  ...
                  {relocateState === "success" ? (
                    <div className="bg-green-500/10 border border-green-500/20 rounded-[2rem] p-10 flex flex-col items-center justify-center text-center">
                      ...
                      <h4 className="text-2xl font-black text-green-600 uppercase tracking-tight mb-2">
                        Request Submitted
                      </h4>
                  ...
                  ) : tracking.bookingStatus !== "PENDING" &&
                    tracking.bookingStatus !== "CONFIRMED" ? (
                    <div className="bg-secondary/50 rounded-[2rem] p-8 text-center text-muted-foreground">
                      Cannot request relocation for a booking that is already{" "}
                      {tracking.bookingStatus}.
                    </div>
                  ) : (
                    <div className="max-w-xl space-y-6">
                      ...
                      {relocateState === "idle" ||
                      relocateState === "sending_otp" ? (
                        <div className="space-y-6 animate-in fade-in duration-500">
                          <div>
                            <label className="text-xs font-black text-foreground uppercase tracking-widest mb-3 block">
                              Available Schedules
                            </label>
                            <select
                            ...
                            >
                              ...
                              {schedules.map((sch) => (
                                ...
                                >
                                  {sch.departureAt
                                    ? format(
                                        parseISO(sch.departureAt),
                                        "PPP 'at' HH:mm",
                                      )
                                    : `Template ID: ${sch.scheduleId}`}
                                </option>
                              ))}
                            </select>
                            {schedules.length === 0 && (
                              <p className="text-xs text-muted-foreground mt-2">
                                No alternative schedules found.
                              </p>
                            )}
                          </div>
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right-8 duration-500">
                          ...
                          <div>
                            <label className="text-xs font-black text-foreground uppercase tracking-widest mb-3 block text-center">
